...
# csmit -m 20G -c 1 "Rscript log2_ChIPinput_per10kb_commandArgs_060418.R ../../arp6_SPO11-oligo_RPI32_nakedDNA_R1_norm_allchrs_coverage_coord_tab.bed arp6_SPO11_oligo_RPI32 nakedDNA_R1_arp6_SPO11_oligo_RPI32"
...
ChIPname <- args[2]
inputname <- args[3]
outDir <- paste0(dirname(covFile), "/log2ChIPinput/genomeProfiles/") 
...
for(s in 1:length(windows)) {
  ...
  cuminputwinDat <- NULL
    for(i in 1:5) {
      ...
      write.table(ChIPwinDat, file = paste0(outDir, ChIPname, "_chr", i, "_norm_coverage_", winNames[s], ".txt"))
      write.table(inputwinDat, file = paste0(outDir, inputname, "_chr", i, "_norm_coverage_", winNames[s], ".txt"))
      ...
      write.table(log2trans, file = paste0(outDir, "log2_", ChIPname, "_chr", i, "_norm_coverage_", winNames[s], ".txt"))
    }
    write.table(cumChIPwinDat, file = paste0(outDir, ChIPname, "_genome_norm_coverage_", winNames[s], ".txt"))
    write.table(cuminputwinDat, file = paste0(outDir, inputname, "_genome_norm_coverage_", winNames[s], ".txt"))
    ...
    write.table(cumlog2trans, file = paste0(outDir, "log2_", ChIPname, "_genome_norm_coverage_", winNames[s], ".txt"))
}
